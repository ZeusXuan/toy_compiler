...
// 自定义的体系结构只有一个通用寄存器,其他时候都使用stack
int * pc,           // pc register
    * sp,           // rsp register
    * bp;           // rbp register
...
// 分词,在词法分析的过程中只处理symbol table中的Name,Token,Hash这三个域
void tokenize() {
    // ch_ptr用于标识symbol的开始位置
    char* ch_ptr;
    while((token = *src++)) {
        // line 行数用来 debug
        if (token == '\n') line++;
        ...
        // handle string & char(处理字符串字面量存储在data段)
        // 只支持双引号
        else if (token == '"' || token == '\'') {
            ch_ptr = data;
...
// 执行嵌套:parse_fun->parse_stmt->parse_exp
void parse_fun() {
    int type, i;
...
int run_vm(int argc, char** argv) {
    ...
    *--sp = (int)tmp;
    ...
    while (1) {
        ...
        else if (op == LC)      ax = *(char*)ax;                // load char(从ax中取地址)
        else if (op == LI)      ax = *(int*)ax;                 // load int
        else if (op == SC)      *(char*)*sp++ = ax;             // save char(从栈上取地址)
